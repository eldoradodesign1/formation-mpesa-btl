...
import { ClipboardCheck, Eye, Loader2, X } from "lucide-react";
...
export function TrainingLogin({ onSuccess, onGuest, notice = "" }: { onSuccess: (token: string, user: TrainingUser) => void; onGuest: () => void; notice?: string }) {
  ...
  return <main className="training-login" aria-label="Connexion à la formation M-Pesa BTL"><section className="training-login__card"><div className="training-login__guest-access"><span>Visiter la formation</span><button type="button" className="training-login__guest" onClick={onGuest} aria-label="Accéder à la formation en mode Invité, lecture seule" title="Consulter en invité — lecture seule"><Eye size={18} strokeWidth={1.8} /></button></div><div className="training-login__mark" aria-hidden="true"><i /><i /><i /></div><span className="micro-label">BTL Learning · Accès formation</span><h1>Entrez dans le parcours.</h1><p>Utilisez les mêmes identifiants que votre espace BTL Deployment Tracker.</p>{notice && <p className="training-login__notice" role="status">{notice}</p>}<form onSubmit={submit}><label><span>Numéro de téléphone</span><input value={phone} onChange={(event) => setPhone(event.target.value)} inputMode="tel" autoComplete="tel" placeholder="Ex. 081…" required /></label><label><span>Mot de passe</span><input value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete="current-password" placeholder="Votre mot de passe" required /></label>{error && <p className="training-login__error" role="alert">{error}</p>}<button type="submit" disabled={loading}>{loading ? <><Loader2 size={16} className="spin" /> Connexion…</> : "Accéder à la formation"}</button></form><div className="training-login__join"><img src={joinQrCode} alt="QR code pour rejoindre la formation M-Pesa BTL" /><p><b>Rejoindre la formation</b><span>Scannez ce code avec votre téléphone.</span></p></div><small>La progression et les tests sont enregistrés automatiquement par module.</small></section></main>;
}
