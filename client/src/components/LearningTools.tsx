...
function exportRows(dashboard: SupervisorDashboard) {
  return dashboard.agents.map((agent) => ({
    ...
    "Cotation superviseur": agent.supervisorRating === null ? "—" : `${agent.supervisorRating}/5`,
    "Score consolidé": agent.consolidatedScore === null ? "—" : `${agent.consolidatedScore}%`,
...
export function SupervisorPanel({ dashboard, onClose, onRate }: { dashboard: SupervisorDashboard; onClose: () => void; onRate: (agentId: string, rating: number, comment: string) => Promise<void> }) {
  ...
  return <aside className="supervisor-panel" role="dialog" aria-modal="true" aria-label="Tableau de bord superviseur"><div className="learning-panel__heading"><div><span className="micro-label">{dashboard.scope === "global" ? "Vision globale" : "Agents affectés"}</span><h2>Suivi de formation</h2></div><button type="button" onClick={onClose} aria-label="Fermer le tableau de bord"><X size={19} /></button></div><div className="supervisor-metrics"><div><span>Personnes suivies</span><b>{dashboard.agents.length}</b></div><div><span>Parcours terminés</span><b>{completed}</b></div><div><span>Score moyen</span><b>{average}%</b></div></div><div className="supervisor-actions"><button type="button" onClick={() => downloadCsv(rows)}><Download size={15} /> CSV</button><button type="button" onClick={() => downloadExcel(rows)}><FileSpreadsheet size={15} /> Excel</button></div>{dashboard.agents.length === 0 ? <div className="supervisor-empty"><UsersRound size={28} /><p>Aucun utilisateur n’est actuellement visible dans votre périmètre.</p></div> : <div className="supervisor-table"><table><thead><tr><th>Participant</th><th>Parcours</th><th>Tests</th><th>Score consolidé</th><th>Cotation</th><th>Activité</th></tr></thead><tbody>{dashboard.agents.map((agent) => <tr key={agent.id}><td><b>{agent.fullName || "Utilisateur non renseigné"}</b><small>{agent.phone} · {agent.category}</small></td><td>{agent.completedModules}/{agent.totalModules}</td><td>{agent.validatedModules}/{agent.totalModules}</td><td>{agent.consolidatedScore === null ? "—" : `${agent.consolidatedScore}%`}</td><td>{dashboard.canRate ? <RatingControl agentId={agent.id} initial={agent.supervisorRating} comment={agent.supervisorComment} onRate={onRate} /> : agent.supervisorRating === null ? "—" : `${agent.supervisorRating}/5`}</td><td>{agent.lastActivity ? new Date(agent.lastActivity).toLocaleDateString("fr-FR") : "—"}</td></tr>)}</tbody></table></div>}</aside>;
}

function RatingControl({ agentId, initial, comment, onRate }: { agentId: string; initial: number | null; comment: string | null; onRate: (agentId: string, rating: number, comment: string) => Promise<void> }) {
  const [rating, setRating] = useState(String(initial ?? 3)); const [note, setNote] = useState(comment ?? ""); const [saving, setSaving] = useState(false);
  return <div className="rating-control"><select aria-label="Cotation superviseur" value={rating} onChange={(event) => setRating(event.target.value)}>{[1,2,3,4,5].map((value) => <option value={value} key={value}>{value}/5</option>)}</select><input aria-label="Commentaire de cotation" value={note} maxLength={600} placeholder="Commentaire" onChange={(event) => setNote(event.target.value)} /><button type="button" disabled={saving} onClick={async () => { setSaving(true); try { await onRate(agentId, Number(rating), note); } finally { setSaving(false); } }}>{saving ? "…" : "Valider"}</button></div>;
}
